	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			.each(function () {
				...
				MultiFile.E.wrap('<div class="MultiFile-wrap" id="wrap_' + MultiFile.wrapID + '"></div>');
				MultiFile.wrapper = $('#wrap_' + MultiFile.wrapID + '');
				...
				if (!MultiFile.list) {
					...
					MultiFile.wrapper.append('<div class="MultiFile-list bootstrap-tagsinput" id="' + MultiFile.wrapID + '_list"></div>');
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ).replace(/[&<>'"]/g, function(c) { return '&#'+c.charCodeAt()+';'; }),
								...
								label =	$(
										(
											'<span class="MultiFile-label tag label label-primary" title="' + t + '">'+
...
//							r = $('<div class="MultiFile-label"></div>'),
							r = $('<span class="MultiFile-label tag label label-primary"></span>'),
